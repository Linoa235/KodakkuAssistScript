...
[ScriptType(name: "LV.80 Eden's Verse: Iconoclasm", territorys: [904], guid: "cbcef663-c201-4d8b-bfe8-6399d66120a2",
...
public class Edens_Verse_Iconoclasm
{
    ...
    [ScriptMethod(name: "Empty Wave", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(19538|20052)$"])]
    public void EmptyWave(Event @event, ScriptAccessory accessory)
    {
        ...
        {
            if (isText) accessory.Method.TextInfo("Heavy AOE", duration: 3500, true);
            accessory.TTS("Heavy AOE", isTTS, isDRTTS);
        }
    }

    [ScriptMethod(name: "Unshadowed Stake", eventType: EventTypeEnum.Tether, eventCondition: ["Id:0025"])]
    public void UnshadowedStake(Event @event, ScriptAccessory accessory)
    {
        if (@event.TargetId() == accessory.Data.Me)
        {
            if (isText) accessory.Method.TextInfo($"Guide the laser to the edge of the arena", duration: 4500, true);
            accessory.TTS("Guide the laser to the edge of the arena", isTTS, isDRTTS);
        }
        else
        {
            if (isText) accessory.Method.TextInfo($"Stay away from the tethered player", duration: 4500, true);
            accessory.TTS("Stay away from the tethered player", isTTS, isDRTTS);
        }
    ...
    [ScriptMethod(name: "Lights and Darkness Course", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(19516|20067|19518|19517|19491|19490|19521)$"])]
    public void LightsAndDarknessCourse(Event @event, ScriptAccessory accessory)
    {
        lock (LightsCourseLock)
        {
            ...
            if (LightsCourseCount == 8 || LightsCourseCount == 9 || LightsCourseCount == 10 || LightsCourseCount == 11)
            {
                ...
                {
                    accessory.Method.TextInfo("Prioritize dodging red", duration: 4700, true);
                    accessory.TTS("Prioritize dodging red", isTTS, isDRTTS);
                }
                ...
                LightsCourseCount == 30 || LightsCourseCount == 31)
            {
                ...
                if (@event["ActionId"] == "19516" || @event["ActionId"] == "19490") 
                {
                    ...
                    {
                        var index = FindExactPositionIndex(@event.SourcePosition(), BallCheckList);
                        DrawHelper.DrawRect(accessory, @event.SourcePosition(), centerPoint[index], new Vector2(10, 50), 5000, "Light's Course 12-31", new Vector4(1, 0, 0, 1));
                    }
                    else 
                    {
                        var index = FindExactPositionIndex(@event.SourcePosition(), BallCheckList);
                        DrawHelper.DrawRect(accessory, @event.SourcePosition(), centerPoint[index], new Vector2(10, 50), 5000, "Light's Course 12-31", new Vector4(0, 1, 0, 1));
                    }
                }
            ...
            if (LightsCourseCount == 24 || LightsCourseCount == 25 || LightsCourseCount == 26 || LightsCourseCount == 27)
            {
                ...
                if (@event["ActionId"] == "19521")
                {
                    ...
                    {
                        var index = FindExactPositionIndex(@event.SourcePosition(), BallCheckList);
                        DrawHelper.DrawRect(accessory, @event.SourcePosition(), centerPoint[index], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(1, 0, 0, 1));
                        if (index == 1) 
                        {
                            DrawHelper.DrawRect(accessory, BallStartList[5], centerPoint[5], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(0, 1, 0, 1), delay: 6700);
                            DrawHelper.DrawRect(accessory, BallStartList[7], centerPoint[7], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(0, 1, 0, 1), delay: 6700);
                        }
                        else if (index == 2)
                        {
                            DrawHelper.DrawRect(accessory, BallStartList[4], centerPoint[4], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(0, 1, 0, 1), delay: 6700);
                            DrawHelper.DrawRect(accessory, BallStartList[6], centerPoint[6], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(0, 1, 0, 1), delay: 6700);
                        }
                    }
                    else
                    {
                        var index = FindExactPositionIndex(@event.SourcePosition(), BallCheckList);
                        DrawHelper.DrawRect(accessory, @event.SourcePosition(), centerPoint[index], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(0, 1, 0, 1));
                        if (index == 1) 
                        {
                            DrawHelper.DrawRect(accessory, BallStartList[5], centerPoint[5], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(1, 0, 0, 1), delay: 6700);
                            DrawHelper.DrawRect(accessory, BallStartList[7], centerPoint[7], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(1, 0, 0, 1), delay: 6700);
                        }
                        else if (index == 2)
                        {
                            DrawHelper.DrawRect(accessory, BallStartList[4], centerPoint[4], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(1, 0, 0, 1), delay: 6700);
                            DrawHelper.DrawRect(accessory, BallStartList[6], centerPoint[6], new Vector2(10, 50), 6700, "Light's Course 24-27", new Vector4(1, 0, 0, 1), delay: 6700);
                        }
                    }
                }
    ...
    [ScriptMethod(name: "Forced Transfer", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:regex:^(2242|2243|2240|2241)$"])]
    ...
    [ScriptMethod(name: "Group Stack", eventType: EventTypeEnum.TargetIcon, eventCondition: ["Id:003E"])]
    public async void Stack(Event @event, ScriptAccessory accessory)
    {
        await Task.Delay(50);
        if (StackCount == 0) 
        {
            accessory.Method.TextInfo("Group stack", duration: 4700, true);
            accessory.TTS("Group stack", isTTS, isDRTTS);
        }
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "Stack";
