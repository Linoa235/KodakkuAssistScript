...
[ScriptType(name: "Rubicante [Extreme]", territorys: [1096],
...
public class RubicanteEx
{
    ...
    """
    v0.0.0.4:
    Unfortunately, the large wheel requires Imgui.
    Duckmen.
    """;
    ...
    const uint HALF_CLEAVE = 15760;       // Outer magic circle red half-room cleave SourceDataId
    const uint FAN = 15759;               // Outer magic circle blue fan SourceDataId
    ...
    #region Large Wheel
    [ScriptMethod(name: "Purgatorial Magic Circle Outer Type Record (uncontrolled)", eventType: EventTypeEnum.SetObjPos, eventCondition: ["SourceDataId:regex:^(157(59|60))$"], userControl: false)]
    ...
    [ScriptMethod(name: "Large Wheel Algorithm", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33001"], userControl: false)]
    ...
    [ScriptMethod(name: "Large Wheel Parameter Initialization (uncontrolled)", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:33001"], userControl: false)]
    ...
    #region Gale Flame Flow

    [ScriptMethod(name: "Gale Flame Flow: Spread, Pair Stack, Four-Player Stack", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(3200[234])$"])]
    public void SpreadFlame(Event @event, ScriptAccessory accessory)
    {
        var tid = @event.TargetId();
        var aid = @event.ActionId();
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"Gale Flame Flow {tid}";
        dp.Owner = tid;
        ...
        {
            { 32002, (5f, ColorHelper.colorPink.V4) },   // Solo spread
            { 32003, (6f, accessory.Data.DefaultSafeColor) }, // Four-player stack
            { 32004, (4f, accessory.Data.DefaultSafeColor) }  // Pair stack
        };
    ...
    [ScriptMethod(name: "Gale Flame Flow: Fan Spread", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:31998"])]
    public void FanSpread(Event @event, ScriptAccessory accessory)
    {
        ...
        {
            dp.Name = $"Fan Flame Flow {i}";
            dp.TargetObject = accessory.Data.PartyList[i];
            accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Fan, dp);
        }
    }

    [ScriptMethod(name: "Cone Tankbuster", eventType: EventTypeEnum.TargetIcon, eventCondition: ["Id:00E6"], userControl: false)]
    public void Dualfire(Event @event, ScriptAccessory accessory)
    {
        var tid = @event.TargetId();
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"Cone Tankbuster {tid}";
        dp.Scale = new(60);
    ...
    [ScriptMethod(name: "Flamespire Buff Spread", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:3485"])]
    public void FlamespireSpreadBuff(Event @event, ScriptAccessory accessory)
    {
        var tid = @event.TargetId();
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"Flamespire Buff Spread";
        dp.Owner = tid;
        dp.Delay = 10000;
        dp.DestoryAt = 5000;
        dp.Scale = new(6);
    ...
    [ScriptMethod(name: "Flame Flow Circle Spread", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:32016"])]
    public void InfernoSpread(Event @event, ScriptAccessory accessory)
    {
        var tid = @event.TargetId();
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"Flame Flow Circle Spread";
        dp.Owner = tid;
        dp.DestoryAt = 5000;
        dp.Scale = new(5);
        dp.Color = accessory.Data.DefaultDangerColor.WithW(1.5f);
    ...
    [ScriptMethod(name: "Scalding Fleet Heavy/Donut Spread", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(3203[67])$"])]
    public void ScaldingFleet(Event @event, ScriptAccessory accessory)
    {
        var sid = @event.SourceId();
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"Scalding Fleet";
        dp.Owner = sid;
        dp.Delay = 0;
        dp.DestoryAt = 5000;
        dp.Color = accessory.Data.DefaultDangerColor.WithW(1.5f);
    ...
    [ScriptMethod(name: "Immolation Charge Return", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:32039"])]
    public void ImmolationChargeBack(Event @event, ScriptAccessory accessory)
    {
        var sid = @event.SourceId();
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"Swift Flame Return";
        dp.Owner = sid;
        dp.Delay = 0;
        dp.DestoryAt = 6500;
        dp.Scale = new(6, 60);
    ...
    [ScriptMethod(name: "Immolation Spread/Stack", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(3203[45])$"])]
    public void ImmolationSpreadStack(Event @event, ScriptAccessory accessory)
    {
        var tid = @event.TargetId();
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"Immolation Spread/Stack";
        dp.Owner = tid;
        dp.Delay = 0;
        dp.DestoryAt = 7500;
    ...
    [ScriptMethod(name: "Immolation Half-Room Cleave", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(3203[23])$"])]
    public void ImmolationHalfCleave(Event @event, ScriptAccessory accessory)
    {
        var sid = @event.SourceId();
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"Immolation Half-Room Cleave";
        dp.Owner = sid;
        dp.Delay = 0;
        dp.DestoryAt = 7000;
        dp.Scale = new(40, 20);
