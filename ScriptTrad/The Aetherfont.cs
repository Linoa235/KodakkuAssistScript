...
[ScriptType(guid: "49da631e-ac61-449e-8c01-45283ffcc734", name: "The Aetherfont", territorys: [1126], version: "0.0.0.2", author: "Linoa235")]
public class TheAetherfont
{
    ...
    [ScriptMethod(name: "B1-Tankbuster", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33345"])]
    public void Boss1Tankbuster(Event evt, ScriptAccessory sa)
    {
        sa.Method.TextInfo("Tankbuster", 5000);
    }
    
    [ScriptMethod(name: "B1-Stack", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33343"])]
    public void Boss1Stack(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("B1-Stack", evt.TargetId(), 5000, 6, true);
        sa.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }
    
    [ScriptMethod(name: "Resonant Frequency", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33339"])]
    public void ResonantFrequency(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("Resonant Frequency", evt.SourceId(), 5000, 8);
        sa.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }

    [ScriptMethod(name: "Explosive Frequency", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33340"])]
    public void ExplosiveFrequency(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("Explosive Frequency", evt.SourceId(), 10000, 15);
    ...
    [ScriptMethod(name: "Tidal Breath", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33344"])]
    public void TidalBreath(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("Tidal Breath", evt.SourceId(), 5000, 40);
        dp.Radian = float.Pi;
    ...
    [ScriptMethod(name: "B2-Tankbuster", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33368"])]
    public void RipperClaw(Event evt, ScriptAccessory sa)
    {
        sa.Method.TextInfo("Tankbuster", 5000);
    }
    
    [ScriptMethod(name: "Spun Lightning", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33363"])]
    public void SpunLightning(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("Spun Lightning", evt.SourceId(), 3500, new Vector2(8, 30));
    ...
    [ScriptMethod(name: "Forked Fissures", eventType: EventTypeEnum.EnvControl, eventCondition: ["Id:regex:^(00020001|00200010)$"])]
    public void ForkedFissures(Event evt, ScriptAccessory sa)
    {
        if (Patterns.TryGetValue(evt.Index(), out var pattern))
        {
            ...
            for (var i = starts.Length - 1; i >= 0; --i)
            {
                var start = starts[i];
                var end = ends[i];
                var dp = sa.FastDp("Forked Fissures", new Vector3(start.X, 20, start.Y), 6000, 4);
    ...
    [ScriptMethod(name: "Clearout", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33348"])]
    public void Clearout(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("Clearout", evt.SourceId(), 9000, 16);
        dp.Radian = float.Pi / 3 * 2;
        sa.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Fan, dp);
    }
    
    [ScriptMethod(name: "Saline Spit", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33353"])]
    public void SalineSpit(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("Saline Spit", evt.SourceId(), 6000, 8);
        sa.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }
    
    [ScriptMethod(name: "Telekinesis", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:33351"])]
    public void Telekinesis(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("Telekinesis", evt.SourceId(), 10000, 12);
        sa.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }
    
    [ScriptMethod(name: "Breath", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(34551|33354)$"])]
    public void Breath(Event evt, ScriptAccessory sa)
    {
        var dp = sa.FastDp("Breath", evt.SourceId(), evt.ActionId() > 33354 ? 16500u : 10000u, 35);
        dp.Radian = float.Pi;
        sa.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Fan, dp);
    }
