...
[ScriptType(name: "LV.20 Stinging Back", territorys: [192], guid: "d9448101-c942-44de-a50d-6ab2df5aa04f",
...
public class Stinging_Back
{
    ...
    [ScriptMethod(name: "Target Display", eventType: EventTypeEnum.AddCombatant, eventCondition: ["DataId:regex:^(595|145|148|519|146|144|147)$"])]
    public async void showTarget(Event @event, ScriptAccessory accessory)
    {
        ...
        lock (showTargetLock)
        {
            ...
            {
                if (isText) accessory.Method.TextInfo("Prioritize attacking red medics", duration: 5000, true);
                accessory.TTS("Prioritize attacking red medics", isTTS, isDRTTS);
            }
            if (@event.DataId() == 595)
            {
            ...
            {
                if (isText) accessory.Method.TextInfo("Focus attack on Redbelly Swarmfly", duration: 5000, true);
                accessory.TTS("Focus attack on Redbelly Swarmfly", isTTS, isDRTTS);
    ...
    [ScriptMethod(name: "Remove Draw", eventType: EventTypeEnum.RemoveCombatant, eventCondition: ["DataId:regex:^(595|145|148|519|146|144|147)$"])]
    public async void delDraw(Event @event, ScriptAccessory accessory)
    {
        accessory.Method.RemoveDraw($"{@event.SourceId()}");
    }

    [ScriptMethod(name: "Heartstopper", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:866"])]
    public void Heartstopper(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "Heartstopper";
    ...
    [ScriptMethod(name: "Remove Draw1", eventType: EventTypeEnum.Director, eventCondition: ["Command:00000001", "Instance:00180001"])]
