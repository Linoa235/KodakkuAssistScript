...
[ScriptType(name: "LV.90 The Dead Ends", territorys: [], guid: "d8c3a05e-65a0-44e9-8814-5067aa90f572", version: "0.0.0.1", Author: "Linoa235")]
...
public class the_Dead_Ends
{
    ...
    public void Boss2TankBuster(Event ev, ScriptAccessory sa)
    {
        ...
        if (ev.TargetId == sa.Data.Me)
        {
            string msg = language == Language.Chinese ? "Laser tankbuster" : "Laser buster";
            if (isText) sa.Method.TextInfo($"{msg}", duration: 4700, true);
            if (isTTS) sa.Method.EdgeTTS($"{msg}");
        }
        else
        {
            string msg = language == Language.Chinese ? "Avoid tankbuster laser" : "Avoid buster Laser";
            if (isText) sa.Method.TextInfo($"{msg}", duration: 4700, true);
            if (isTTS) sa.Method.EdgeTTS($"{msg}");
        }
...
// (Helper classes remain the same as previous files - DrawHelper, EventExtensions, etc.)
// ... (IbcHelper, DrawHelper, MathTools, IndexHelper, MarkerHelper, etc.)
#endregion