...
[ScriptType(name: "LV.15 Basic Training: Enemy Strongholds", territorys: [215], guid: "6d433426-d546-48bd-b4c5-0a08cbe5ec43",
...
public class Basic_Training_Enemy_Strongholds
{
    ...
    [ScriptMethod(name: "Labored Leap", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:1006"])]
    public void LaboredLeap(Event @event, ScriptAccessory accessory)
    {
        if (isText) accessory.Method.TextInfo("Chariot, move away from boss", duration: 4000, true);
        accessory.TTS("Chariot, move away from boss", isTTS, isDRTTS);

        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "Labored Leap";
    ...
    [ScriptMethod(name: "Lever Drawing Indication", eventType: EventTypeEnum.MorelogCompat, eventCondition: ["MorlogId:101"])]
    public async void Notify(Event @event, ScriptAccessory accessory)
    {
        ...
        lock (NotifyLock)
        {
            if (NotifyCount == 0)
            {
                ...
                {
                    var dp = accessory.Data.GetDefaultDrawProperties();
                    dp.Name = $"Lever Drawing Indication{i}";
    ...
    [ScriptMethod(name: "Remove Draw", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:870"])]
