...
[ScriptType(name: "LV.100 Yuweyawata Field Station", territorys: [], guid: "d57753ce-5a55-4aac-bf07-8bc5a799084c", version: "0.0.0.1", Author: "Linoa235")]

public class YuweyawataFieldStation
{
    //^(?!.*((Monk|Machinist|Dragoon|Scholar|Dancer|Viper|Dark Knight|(Eos|Selene)|Seraph|White Mage|Warrior|Paladin|Summoner|Carbuncle|Demiphoenix|Demibahamut|Garuda-Egi|Titan-Egi|Ifrit-Egi|Queen Automaton)\] (Used|Cast|Cancel|Add))).*$
    ...
    """
    v0.0.2.1:
    1. If there are any missing drawings (boss appearing in the south side nuclear explosion) or incorrect drawings, please @ me on discord and attach the arr file
    Duckmen.
    """;
    ...
    public void RawElectrope(Event @event, ScriptAccessory accessory)
    {
        ...
        {
            if (isText) accessory.Method.TextInfo("Prioritize attacking the add", duration: 4700, true);
            accessory.TTS("Prioritize attacking the add", isTTS, isDRTTS);
            RawElectropeCount++;
        }
    }
    ...
    #region Boss2
    [ScriptMethod(name: "Boss2 Tankbuster", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:40658"])]
    ...
    {
        if (isText) accessory.Method.TextInfo("Go under the boss", duration: 4700, true);
        accessory.TTS("Go under the boss", isTTS, isDRTTS);
    ...
    public void TelltaleTears(Event @event, ScriptAccessory accessory)
    {
        ...
        {
            if (isText) accessory.Method.TextInfo("Spread, do not overlap", duration: 4700, true);
            accessory.TTS("Spread, do not overlap", isTTS, isDRTTS);
    ...
    {
        if (isText) accessory.Method.TextInfo("Move away from the center, watch the eye", duration: 4700, true);
        accessory.TTS("Move away from the center, watch the eye", isTTS, isDRTTS);
    ...
    {
        if (isText) accessory.Method.TextInfo("Go behind the boss", duration: 4700, true);
    ...
    {
        if (isText) accessory.Method.TextInfo("Move away from the boss", duration: 7000, true);
        accessory.TTS("Move away from the boss", isTTS, isDRTTS);
    ...
    [ScriptMethod(name: "Boss3 Tankbuster", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:40619"])]
    ...
    [ScriptMethod(name: "Rock Blast - Draw", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:40611"])]
    ...
    [ScriptMethod(name: "Rock Blast - Navigation", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:40611"])]
            ...
            switch (@event.SourceRotation())
            {
                ...
                case -0f:
                    ...
                    if (Spos0.X >= 45 && Spos0.Z <= -700 && Spos0.Z >= -720)
                    {
                        ...
                        dp4.Name = "Rock BlastNavi rect right Half 0";
                    ...
                    } else
                    {
                        ...
                        dp3.Name = "Rock BlastNavi rect right Half 0";
